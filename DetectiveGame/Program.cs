...
class Program
{
    ...
    {
        Console.Title = "Детективное расследование";
        ...
        while (true)
        {
            ...
            {
                gameState = saveLoadService.LoadGame();
                isNewGame = gameState.Locations.Count == 0;

                if (isNewGame)
                    InitializeNewGame(gameState);
            }

            var gameService = new GameService(gameState, saveLoadService);

            if (isNewGame)
                gameService.ShowIntro();

            var result = gameService.StartGameLoop();

            if (result == GameLoopResult.Restart)
            {
    ...
    static void InitializeNewGame(GameState gameState)
    {
        // Локации
